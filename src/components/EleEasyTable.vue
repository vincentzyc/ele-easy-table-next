<template>
  <div class="ele-easy-table" style="overflow: hidden;">
    <el-form :class="form.class" :inline="true" :model="formData" :style="{ position: 'relative', ...form.style }"
      v-bind="form.config" v-if="Object.keys(form).length > 0">
      <span :key="key" v-for="(item, key) in form.list" v-show="showFormItem(item, key)">
        <el-form-item :label="item.label ? item.label + '：' : ''" v-bind="item.formConfig" v-if="item.type !== 'slot'">
          <el-date-picker :style="item.style || form.formItemStyle" @change="getDate(item)" end-placeholder="结束日期"
            start-placeholder="开始日期" type="daterange" v-bind="item.config" v-if="item.type === 'datePicker'"
          ...
          <el-input :placeholder="item.placeholder ? item.placeholder : '请输入' + item.label"
            :style="item.style || form.formItemStyle" @keyup.enter="$emit('get-list')" v-bind="item.config"
            v-if="item.type === 'input'" v-model="formData[item.key]"></el-input>
          <el-select :placeholder="item.placeholder ? item.placeholder : '请选择' + item.label"
            :style="item.style || form.formItemStyle" v-bind="item.config" v-if="item.type === 'select'"
            v-model="formData[item.key]">
            <el-option :key="typeof (option.value) !== 'undefined' ? option.value : option"
              :label="typeof (option.label) !== 'undefined' ? option.label : option"
              :value="typeof (option.value) !== 'undefined' ? option.value : option" v-for="option in item.options">
            </el-option>
          </el-select>
          <el-button :disabled="item.disabled ? item.disabled() : false" :style="item.style"
            @click="item.handleClick(item, key)" type="primary" v-bind="item.config" v-if="item.type === 'button'">{{
    ...
    <el-table :data="table.list" border element-loading-text="拼命加载中" stripe style="overflow: visible;margin-top:20px;"
      tooltip-effect="light" v-bind="$attrs" v-if="Object.keys(table).length > 0" v-loading="table.isLoading">
      <el-table-column align="center" type="selection" v-bind="table.selection.config"
        v-if="table.selection && table.selection.show && table.list.length > 0"></el-table-column>
      <el-table-column :fixed="table.indexFixed || false" :label="table.indexLabel || '序号'" align="center" type="index"
        :index="tableIndex" v-if="table.showIndex !== false" width="55"></el-table-column>
      <el-table-column :key="column.key + column.label" :label="column.label" :prop="column.key" align="center"
        v-bind="column.config" v-for="column in table.columns">
        ...
        <template #default="scope">
          ...
          <span v-if="column.type === 'textBtn'">
            <span :key="key" v-for="(btn, key) in column.textBtn">
              <el-button @click="btn.handleClick(scope.row, scope)" style="margin-right:10px" type="primary" link
                v-bind="btn.config || btn.funcConfig ? btn.funcConfig(scope.row, scope) : {}"
                v-html="btn.text || btn.funcText(scope.row, scope)" v-if="btn.text || btn.funcText(scope.row, scope)">
              </el-button>
            </span>
          </span>
    ...
    <div style="float: right;margin: 20px 0;"
      ...
      <el-pagination :current-page="formData.pageIndex || 1"
        :layout="typeof (pagination) === 'object' ? pagination.layout : 'total, sizes, prev, pager, next, jumper'"
        :page-size="formData.pageSize || 10"
        :page-sizes="typeof (pagination) === 'object' ? pagination.pageSizes : [10, 20, 50, 100]" :total="11"
        @current-change="handleCurrentChange" @size-change="handleSizeChange"
        v-bind="typeof (pagination) === 'object' ? pagination : {}"></el-pagination>
    </div>
  </div>
</template>
