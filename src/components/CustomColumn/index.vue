...
function searchColumns(word = "") {
  showColumns.value = allColumns.value
    .filter((val) => val.label.toUpperCase().includes(word.toUpperCase()))
    .map((v) => v.key);
...
async function initLocalStorage() {
  ...
  if (Array.isArray(checkedColumns) && checkedColumns.length > 0) {
    let hadCheckedColumns: TypeColumnsItem[] = [],
      checkedColumn: TypeColumnsItem | undefined;
    checkedColumns.forEach((v, i) => {
      checkedColumn = props.baseColumns?.find((item) => item.key === v);
      checkedColumn && checkedColumn.key
        ? hadCheckedColumns.push(checkedColumn)
        : checkedColumns.splice(i, 1);
    });
...
function handleCheckAllChange(bool: boolean) {
  const showColumnsKeys = bool
    ? showColumns.value
    : showColumns.value.filter((v) => !disabledCustoms.value.includes(v));
